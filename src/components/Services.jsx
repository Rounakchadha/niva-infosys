          ...
          <div className="flex w-max animate-marquee py-4">
            ...
            <div className="flex gap-6 px-3">
              {services.map((svc) => (
                <div
                  key={`set1-${svc.title}`}
                  className="w-[320px] sm:w-[380px] shrink-0 group relative bg-zinc-900/40 border border-white/5 hover:border-blue-500/30 rounded-3xl p-8 card-hover cursor-default overflow-hidden transition-all duration-300"
                >
                  <div className="w-12 h-12 bg-blue-600/10 rounded-2xl flex items-center justify-center mb-6 transition-transform group-hover:scale-105 group-hover:bg-blue-600/20">
                    <svc.icon className="w-5 h-5 text-blue-500" />
                  </div>
                  <h3 className="text-white font-semibold text-xl mb-3 tracking-tight">{svc.title}</h3>
